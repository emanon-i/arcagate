...
- main に直 push OK（PR は大きな変更単位で任意）
- `feat/*` / `fix/*` / `chore/*` / `docs/*` — 通常の prefix、`refactor/` — 大規模整理
- test gate (svelte-check / clippy / cargo test / build) は全 branch で同一
- frontend test (vitest / playwright) は PR-Z で全削除済。T1-T4 plan で再構築中 (`docs/l1_requirements/test-rebuild/` 参照)
...
| 状況                                                       | 読む doc                                              |
| ---------------------------------------------------------- | ----------------------------------------------------- |
| 進行ルール                                                 | `docs/l1_requirements/distribution/dispatch-rules.md` |
| 設計判断 / FE-BE 分担 / エラー方針 / IPC コマンド一覧      | `docs/l2_foundation/engineering-principles.md`        |
| システム構成 / スタック / アーキテクチャ図 / CI/CD         | `docs/l2_foundation/foundation.md`                    |
| Service Layer / IPC 設計 / State / エラー型                | `docs/l2_foundation/foundation-architecture.md`       |
| UI 数値仕様 / レイアウト / widget 仕様 / UI チェックリスト | `docs/l1_requirements/ux-standards.md`                |
| テーマ / トークン                                          | `docs/l1_requirements/design-system.md`               |
| 製品方針 / 要件                                            | `docs/l1_requirements/vision.md`                      |
| 過去の失敗 / 再発防止                                      | `docs/lessons.md`                                     |
| ガイドライン doc 全体地図                                  | `memory/design_guidelines_index.md`                   |
| L0 製品ビジョン / UX / ビジュアル方向                      | `docs/l0_ideas/`                                      |
| 古い retrospective / 達成済 plan                           | `docs/l3_phases/_archive/`                            |
