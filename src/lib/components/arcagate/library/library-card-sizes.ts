 ...
 *
 * LibraryCard 内で 4 個の $derived (iconClassNone / labelPadClass / labelFontClass /
 * targetFontClass) が itemSize にだけ依存していたが、
 * 690 cards × 4 reactive deps が configStore.itemSize 変更で全 trigger され、
 * JS longtask 1.2 秒の主因だった。
 *
 * sizeClasses は親 (LibraryView / LibraryItemPicker) で 1 回だけ derive して props 配布。
 * reactive deps を 690 → 1 に削減。
 */
