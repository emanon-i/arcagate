...
test('exe-folder: popover で別 EXE を選択すると Library item.target が新 path に同期する (PH-CF-1200 ⑧)', async ({
	...
	// WorkspaceLayout を mount して Home auto-create を triger してから列挙する。
	// 単独実行で workspace 未生成のまま polling し続ける regression を防ぐ
	// (`routine-widget.spec.ts` と同じ pattern、 PR #578 では prior spec の副作用に依存していた)。
