...
test('LB-2 (PH-CF-1100 ②): 見た目設定ダイアログから画像変更 → グリッドカード即時切替 (実 UI flow)', async ({
	...
	try {
		...
		//     `<APPDATA>/icons/<uuid v7>.<ext>` を返すため、 seam に渡した `updatedIcon` の
		//     source filename と保存後 path の filename は一致しない。 「initial path を含まなく
		//     なった」 を `not.toContain(initialTail)` で確認することで src が確実に切替った
		//     signal を capture する (これが消えれば ② 「modal 経由で画像変更 → grid 即時反映」
		//     が回帰)。
		await expect
			.poll(
				async () =>
