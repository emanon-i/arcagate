...
async function openLibrary(page: import('@playwright/test').Page): Promise<void> {
	// Tauri default 800x600 で `LibraryLayout` の detail wrapper が `hidden lg:block` で
	// display:none になる。 LB-2 (実 UI 経路) は detail panel 内の toggle / 歯車 / 画像
	// 選択 button を click する必要があるため、 1280x800 へ resize して `lg:` を満たす。
...
test('LB-2 (C2 / PH-CF-1100 ②): 見た目設定ダイアログから画像変更 → グリッドカード即時切替 (実 UI flow)', async ({
	...
	try {
		...
		await page.waitForLoadState('domcontentloaded');
		await page.locator('main').first().waitFor({ state: 'visible', timeout: 30_000 });
		// reload で Tauri window state が reset される CI 環境向けに 再 resize。
		await resizeMainWindow(page, 1280, 800);
		...
		// (6) 画面遷移なしで一覧カードの <img src> が新 path に切替わることを verify。
		//     {#key item.card_override_json|item.icon_path} で再マウントされ <img> 要素は
		//     別 instance になるため locator を再取得して poll する。
