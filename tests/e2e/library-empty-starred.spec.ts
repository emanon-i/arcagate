...
test.describe('Library 空状態・starred バッジ（PH-20260422-023/019 検証）', () => {
	...
	test('DetailPanel で ★ ボタンを押すとカードに starred バッジが表示されること', async ({
		...
		try {
			...
			// ★ ボタンをクリック（お気に入りに追加: CLAUDE.md label rule）
			await detailPanel.getByRole('button', { name: 'お気に入りに追加' }).click();

			// starred バッジが表示される
			await expect(card.getByTestId('starred-badge')).toBeVisible();

			// もう一度クリックでお気に入りを解除 (CLAUDE.md label rule)
			await detailPanel.getByRole('button', { name: 'お気に入りを解除' }).click();
