...
- [x] fireAt 4 パターンの結果記録 (resume 9/10/11/12、`lessons.md` 追記済)
- [x] spawn-on-pressure 利用可否判定 → **default 無効化** (信頼性問題確定)
- [x] dispatch-operation.md §10 更新 (default モード切替 + 信頼性問題明記)
- [x] lessons.md に再発防止策追記
- [ ] auto-kick refine 後の動作実証 transcript 取得は別 plan (auto-kick が send_message のみで kick することを想定された動作確認、cron 走行のみで観測可能なので継続観測扱い)
