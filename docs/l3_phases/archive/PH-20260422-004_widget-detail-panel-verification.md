...
- [x] Favorites / Recent Launches / Projects 全てで右クリックが詳細パネルを開く（コード確認: 全 3 ウィジェットに onItemContext 実装済み）
- [x] 3 ウィジェットのパネルと Library パネルが視覚的に一致（同一 LibraryDetailPanel コンポーネントを使用）
- [x] 削除・編集・タグ付与・実行ボタンが 3 ウィジェットから正常動作（onEditItem 伝播 + Esc ハンドラを修正）
- [x] Esc および 閉じるボタンでパネルが閉じる（svelte:window onkeydown 追加）
- [ ] `tmp/screenshots/PH-20260422-004/` に比較スクショが揃っている（Tauri dev build が computer-use の app allowlist 未登録のため未収集）
- [x] `pnpm verify` 通過
