...
- [x] `pnpm verify` 通過
- [x] `cmd_is_starred` / `cmd_set_starred` IPC コマンドが存在すること（既存 handleToggleStar で実装済み）
- [x] LibraryDetailPanel に ★ トグルボタンが表示されること（既実装済み）
- [x] LibraryCard でスター付きアイテムに ★ バッジが表示されること
