...
- [x] 編集モード ON 時にドットグリッド背景が表示される（radial-gradient）
- [x] ウィジェットをクリックすると選択アウトライン（accent outline）が表示される
- [x] SE リサイズハンドルを 5×5 → 6×6 に拡大（4隅は今回スコープ外）
- [x] ドラッグゴーストを accent pill に差し替え（setDragImage）
- [x] 編集モード OFF 時は装飾が非表示
