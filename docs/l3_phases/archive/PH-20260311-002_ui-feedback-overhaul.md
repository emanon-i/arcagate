...
| 実装順 | #   | 内容                                | 件数 | 状態 |
| ------ | --- | ----------------------------------- | ---- | ---- |
| 1      | S-1 | ヘッダー・ナビゲーション再構成      | 4件  | done |
| 2      | S-2 | Library サイドバー・カード改修      | 4件  | done |
| 3      | S-3 | Library 詳細パネル改修              | 7件  | done |
| 4      | S-4 | モーダル・ダイアログ修正            | 1件  | done |
| 5      | S-5 | 全体設定統合 + Workspace サイドバー | 6件  | done |
| 6      | S-6 | ウィジェット基盤改修                | 6件  | done |
| 7      | S-7 | Workspace 編集機能修復              | 3件  | done |
| 8      | S-8 | ウォッチフォルダーウィジェット改修  | 3件  | done |
...
- [x] TitleBar に "Arcagate" テキストが表示されない
- [x] TitleBar のドラッグ領域・ウィンドウ操作ボタンは維持
- [x] E2E: TitleBar が存在し、ウィンドウ操作ボタン（最小化・最大化・閉じる）が動作する
...
- [x] AppHeader の左側に PanelLeft アイコンボタンが存在しない
- [x] E2E: `[aria-label="Sidebar"]` ボタンが存在しない
...
- [x] タブの上下スペースが視覚的に均等（下側を現在の3倍に拡大）
- [x] vitest snapshot: TitleTab の padding が上下均等
...
- [x] AppHeader 右側にはアクションボタンが存在しない
- [x] AppHeader 左側に設定・可視化・パレットの3ボタンがこの順序で表示
- [x] E2E: 左側に `[aria-label="Settings"]`, `[aria-label*="非表示"]`, `[aria-label="Palette"]` がこの順で存在
...
- [x] アイコン列（48px）が常時表示されている
- [x] 折りたたみ時にタグアイコンクリックでパネルが 200px に展開する
- [x] 展開時に選択中タグの再クリックで 48px に折りたたまれる
- [x] 展開時に別タグクリックでフィルタが切り替わりパネルは開いたまま
- [x] 「すべて」が選択中のとき「すべて」クリックで折りたたまれる
- [x] E2E: サイドバーのタグアイコンクリックで展開/折りたたみをテスト
...
- [x] カード右上に "…" (MoreHorizontal) ボタンが存在しない
- [x] カードクリックで詳細パネルが開き、そこから操作可能
- [x] E2E: LibraryCard に DropdownMenu トリガーが存在しない
...
- [x] LibraryCard のアート領域のアスペクト比が 16:9
- [x] 総アイテム数等の情報カードも同じ 16:9 アスペクト比
- [x] vitest: カードコンポーネントに `aspect-video` クラスが適用されている
...
- [x] ウィンドウ幅が広い場合、カードが横並びできるまで左右に余白が保たれる
- [x] カード幅が `max-w-sm` を超えない
- [x] E2E: ワイド幅（1200px以上）でカード間に適切な余白がある
...
- [x] 非表示状態のとき「プライベート ON」と表示
- [x] 表示状態のとき「プライベート OFF」と表示
- [x] "不可視化" / "可視化" のテキストがどこにも存在しない
- [x] E2E: SensitiveControl のテキストが「プライベート ON」または「プライベート OFF」
...
- [x] アイコンが表示域いっぱいに表示される（空白なし）
- [x] アイコンのアスペクト比が維持されている
- [x] vitest: `object-cover` クラスが適用されている
...
- [x] 長い URL が "…" で省略表示される
- [x] マウスオーバーでツールチップに全文が表示される
- [x] E2E: 長い URL を持つアイテムで `title` 属性に全 URL が設定されている
...
- [x] 長いタイトルが省略表示され、システムタグバッジと重ならない
- [x] タイトルにマウスオーバーで全文ツールチップが表示される
- [x] E2E: タイトル要素に `truncate` クラスと `title` 属性が存在
...
- [x] 詳細パネルにアイテムに紐づくタグがチップ形式で表示される
- [x] タグがない場合は「タグなし」等のプレースホルダーが表示される
- [x] E2E: 詳細パネルにタグセクションが存在する
...
- [x] 詳細パネルからアイテムにタグを追加できる
- [x] 詳細パネルからアイテムのタグを解除できる
- [x] タグ変更が即座に DB に反映される
- [x] E2E: タグの追加・解除操作後にタグ表示が更新される
...
- [x] デフォルトアプリの表示が他の DetailRow と同じスタイル
- [x] ファイル選択ボタンが表示される（フォルダ型アイテムのみ）
- [x] ボタンクリックで OS のファイル選択ダイアログが開き、exe を選択できる
- [x] 選択した exe パスが保存される
- [x] E2E: フォルダ型アイテムの詳細パネルにファイル選択ボタンが存在する
...
- [x] アイテム追加/編集モーダルの背景が透けない
- [x] モーダルの背後のコンテンツが見えない
- [x] E2E: ItemFormDialog を開いた状態で背後の要素が視認不可能（スクリーンショット比較 or opacity チェック）
...
- [x] Workspace 画面にサイドバー（48px アイコン列）が表示される
- [x] 編集アイコンクリックでパネルが展開しウィジェット一覧が表示される
- [x] 展開時にワークスペースが編集モードに入る
- [x] ×ボタンで編集がキャンセルされ編集前に戻る
- [x] ✓ボタンで編集が確定し保存される
- [x] E2E: 編集アイコンクリック → パネル展開 → ウィジェット一覧表示 → ×/✓で閉じる
...
- [x] Workspace 切り替えタブが現状より視認しやすい大きさ
- [x] vitest: Chip の padding/font-size が拡大されている
...
- [x] Library 画面のサイドバー下部に歯車アイコンが表示される
- [x] Workspace 画面のサイドバー下部に歯車アイコンが表示される
- [x] 歯車アイコンクリックで全体設定モーダルが開く
- [x] E2E: サイドバー下部の歯車ボタンクリックで設定モーダルが表示される
...
- [x] PageTabBar に Dark/Light のテーマ切り替えチップが存在しない
- [x] 全体設定モーダル内にテーマ切り替え UI がある
- [x] テーマ切り替えが全体設定から正常に動作する
- [x] E2E: 設定モーダルでテーマ切り替えを実行し、テーマが変更される
...
- [x] Workspace ヘッダーに "…" (MoreMenu) ボタンが存在しない
- [x] ウィジェット追加はサイドバーパネルから行える
- [x] E2E: MoreMenu ボタンが存在しない
...
- [x] 全ウィジェットの右上に "…" 設定アイコンが常時表示される
- [x] アイコンクリックで設定モーダルが開く
- [x] E2E: 各ウィジェットに設定アイコンが存在し、クリックでモーダルが表示される
...
- [x] ウィジェットが固定ピクセルサイズ（16:9 比率）で表示される
- [x] ウィンドウ幅が広がると自動で列数が増える
- [x] Ctrl+マウススクロールでウィジェットの拡大/縮小が動作する
- [x] 拡大率が全体設定で調整可能
- [x] 拡大率がアプリ再起動後も永続化される
- [x] E2E: ウィジェットのアスペクト比が 16:9
...
- [x] ウィジェットの背後のコンテンツが透けて見えない
- [x] E2E: WidgetShell の背景色が不透明（alpha = 1.0）
...
- [x] ウィジェット同士が重なって表示されない
- [x] ウィジェット追加/移動時に重なりが検出された場合、自動で空きスペースに配置される
- [x] E2E: 複数ウィジェットが重ならず表示されること
...
- [x] ウィンドウ外のウィジェットにスクロールでアクセスできる
- [x] 水平スクロールが可能
- [x] 垂直スクロールが可能
- [x] E2E: ウィンドウサイズより大きなコンテンツ配置時にスクロールバーが表示される
...
- [x] ウィジェット内のアイテムを右クリックで右側に詳細パネルが表示される
- [x] 詳細パネルは Library の DetailPanel と同じ内容（起動・編集・削除・タグ等）
- [x] パネル外クリック or × で閉じる
- [x] E2E: ウィジェットアイテム右クリック → 詳細パネル表示 → 閉じる
...
- [x] 編集モードに入れる
- [x] 編集モードでウィジェットの追加・削除が動作する
- [x] 編集モードを終了できる
- [x] E2E: 編集モード → ウィジェット追加 → 確定 → ウィジェットが表示される
...
- [x] 編集モードでウィジェットをドラッグ＆ドロップで移動できる
- [x] 移動後のウィジェット位置が正しく保存される
- [x] E2E: ウィジェットを D&D で移動 → 位置が保持される
...
- [x] 配置済みウィジェットがワークスペース表示時に正しく表示される
- [x] ページリロード後もウィジェットが消えない
- [x] E2E: ウィジェットを配置 → ページ遷移して戻る → ウィジェットが表示されている
...
- [x] ウィジェットタイトルが設定モーダルで変更可能
- [x] デフォルトは「Projects & Git status」以外の適切な名前（例: "ウォッチフォルダー"）
- [x] 変更したタイトルがウィジェットヘッダーに反映される
- [x] E2E: タイトル変更 → ウィジェットヘッダーに反映される
...
- [x] 設定モーダルにタイトル入力フィールドがある
- [x] 設定モーダルに説明入力フィールドがある
- [x] 設定モーダルに監視対象フォルダの選択ボタンがある
- [x] 説明がウィジェットタイトルの下に表示される
- [x] E2E: 設定モーダルで3フィールドが入力可能
...
- [x] 設定モーダルに「自動追加」ON/OFF トグルがある
- [x] ON の場合、監視で見つかったフォルダが Library にアイテムとして自動追加される
- [x] 既に登録済みのフォルダは重複追加されない
- [x] OFF の場合、自動追加は行われない
- [x] E2E: 自動追加 ON → 新しいフォルダが監視ディレクトリに追加 → Library にアイテムが自動登録される
...
- [x] S-1〜S-8 の全受け入れ条件がパス
- [x] `pnpm verify` 通過
- [x] 既存 E2E テスト通過
- [x] 新規 E2E テストで各ステップの受け入れ条件をカバー
- [x] 手動確認で UI の透過・重なり・余白に問題なし
