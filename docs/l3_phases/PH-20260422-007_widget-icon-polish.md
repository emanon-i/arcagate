...
- [x] FavoritesWidget のアイテム行にアイコンが表示される
- [x] RecentLaunchesWidget のアイテム行にアイコンが表示される（cyan dot をアイコンに置換）
- [x] icon_path がない場合にフォールバックアイコン（AppWindow）が表示される（空白なし）
- [x] ダークモード・ライトモードで視認性が確保されている（`text-[var(--ag-text-muted)]` で描画）
- [x] `pnpm verify` 通過
- [ ] スクショを `tmp/screenshots/PH-20260422-007/` に保存（Tauri dev build が computer-use 未登録のため未収集）
