...
-- デフォルト: read-only
INSERT INTO mcp_permissions (id, tool_name, allowed) VALUES
    (generate_uuid(), 'arcagate_list', 1),
    (generate_uuid(), 'arcagate_search', 1),
    (generate_uuid(), 'arcagate_launch', 0),
    (generate_uuid(), 'arcagate_create', 0);
```
...
- [ ] `arcagate_cli mcp` で MCP サーバーが起動し、stdin/stdout で JSON-RPC 通信できる
- [ ] Claude Code から `arcagate_list` でアイテム一覧を取得できる
- [ ] Claude Code から `arcagate_search` でアイテムを検索できる
- [ ] Claude Code から `arcagate_launch` でアイテムを起動できる（write 許可後）
- [ ] Claude Code から `arcagate_create` でアイテムを新規登録できる（write 許可後）
- [ ] write 操作を未許可状態で呼び出すとエラーが返る
- [ ] `arcagate_cli mcp --setup` で設定ファイルが自動更新される
- [ ] `pnpm verify` が全通過する
