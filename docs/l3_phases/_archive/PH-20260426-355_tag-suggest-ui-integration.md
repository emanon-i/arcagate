...
# PH-355: タグ candidate suggest UI 結合（batch-77 PH-341 + batch-78 PH-347 持越）

## 横展開チェック実施済か

- batch-77 で `filterTagSuggestions` / `isExistingTag` 純粋関数化済（vitest 10 件 pass）
- batch-78 でも未着手 → 持ち越し
- LibraryItemTagSection.svelte が既に簡易な追加 UI を持っているので、そこに candidate dropdown を組み込む
...
- [ ] LibraryItemTagSection でタグ入力時に既存タグが suggest される
