...
CLI・ワークスペース・Agent-first CLI により、単なるランチャーから「コントロールパネル」へ進化させる。パワーユーザー向けの操作強化と、AIからの操作インターフェースを確立する。
...
| PH-003-C     | ~~MCP サーバー~~ → PH-003-H で除去（Agent-first CLI に移行）  | F-20260226-015        | —      |
...
| PH-003-G     | shadcn-svelte `import type` バグ修正                          | —（技術的負債）       | —      |
...
### ~~MCP サーバー（PH-003-C で確立）~~ — PH-003-H で除去

> **除去済み**: PH-003-H で MCP サーバーを除去し、Agent-first CLI + Skill に移行。
> 以下は履歴として残す。`src-tauri/src/mcp/` ディレクトリおよび関連コードは削除済み。
...
- Claude CodeからAgent-first CLI経由でアイテムの検索・起動を操作できる
...
| ファイル                  | 追加テーブル                                                              | 追加フェーズ |
| ------------------------- | ------------------------------------------------------------------------- | ------------ |
| `002_mcp_permissions.sql` | `mcp_permissions`（PH-003-H で MCP 除去。マイグレーションファイルは残存） | PH-003-C     |
