...
- **ブラウザ URL を core にしない** — 実機で未検証 (ロック中)。候補扱いのまま。unlocked browser での
  UIA アドレスバー再検証を経てから格上げ判断。本フェーズでは実装しない (§やらないこと固定)
- **UIA の汎用スクレイプをしない** — 編集中テキスト・メール本文・chat 内容を読まない
- **常時ポーリング / ハッシュ / 全走査をしない** — 負荷制約 (REQ-20260702-003) 違反

### ブラウザ URL の扱い (候補・本フェーズ外の明示)

activity-recorder.md 実機表でブラウザ URL は「未検証 (要 unlocked browser 再検証)」。core 5 信号
(窓 / 実操作 / メディア / ファイル操作) を先に固め、URL は候補のまま据え置く。将来格上げする時は
per-browser adapter (Chrome / Edge / Firefox + Chromium 系流用)・取得失敗時「ブラウザ (ドメイン不明)」
フォールバック・ドメイン粒度のみ保存 の方針を activity-recorder.md から引く。**本フェーズでは着手しない**。

## 依存

- 先行: PH-V2-100 (書き込み先の store)
