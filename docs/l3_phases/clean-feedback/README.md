...
| [PH-CF-900](./PH-CF-900_startup-perf-exe-cache.md)      | 起動 perf: exe scan キャッシュ                    | A1                     | 1    |
...
| **キャッシュ無し cold fs walk**            | A1         | exe scan のキャッシュ不在を起点に、 同型の cmd_list_files / cmd_scan_script_folder / auto_register_folder_items も audit                                                                            | PH-CF-900            |
...
2. **projects widget を「別系統」 として比較対象から除外**
   projects (フォルダ監視) widget は `git_poll` 等の独自機能が多く、 PH-PQ-600 では exe_folder / script_folder を主対象に sweep し projects を audit グループから外した。 → 「監視ウィジェット 3 種を 1 つの比較グループとして並べる」 という横展開 (`lateral-sweep` rule) が成立していなかった。

3. **chrome の比較が「コンポーネント有無」 止まりで「prop / 配置」 まで降りていない**
   D3 の実体は WidgetShell に渡す `path` prop の有無 / `icon` の meta 不一致 / description disclosure の配置 (empty state の内 or 外) といった **prop レベルの差**。 PH-PQ-600 の chrome matrix は「header chrome ✓ / settings modal ✓」 の有無チェック粒度で、 prop の値までは比較していなかった。
...
| 9  | PH-CF-900  | 起動 perf。 400 で再設計した scanner にキャッシュを載せる                            | PH-CF-400 |
...
| キャッシュ無し cold fs walk     | `features/backend/exe-scanner.md`                                            | exe scan の P95 perf budget を CI gate に                                                 |
...
- **F1 テーマ並び順**: user 希望順は「ダーク/ライト → ブルータリスト ダーク/ライト → ニューモーフ ダーク/ライト」 の 6 項目だが、 現状の builtin テーマは 5 本 (Dark / Light / Neumorph / Brutalist / HUD) で **dark/light の 2 バリアント構成ではなく HUD が希望リストに無い**。 並び替え機構 (`sort_order`) の導入は PH-CF-800 で確定だが、 「テーマ体系を 6 本へ拡張するのか / 既存 5 本を希望順に近づけて並べるだけか」 は実装着手前に user 確認が要る。 PH-CF-800 §F1 を参照。
