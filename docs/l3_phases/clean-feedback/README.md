...
| PH                                                      | テーマ                                                                       | 含む指摘 ID                     | 件数 |
| ------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------- | ---- |
| [PH-CF-100](./PH-CF-100_workspace-library-integrity.md) | workspace ↔ Library 参照整合 + 監視アイテムの逆方向ライフサイクル data model | E4, E5 (+ 逆方向 LC data model) | 2    |
...
| [PH-CF-500](./PH-CF-500_watch-widget-chrome.md)         | 監視ウィジェット chrome 統一 + 設定デフォルト整合 + 除外アイテム復元 UI      | D1, D2, D3, D4, D7 (+ 復元 UI)  | 5    |
...
| **監視アイテムの逆方向ライフサイクル**     | E6 検討中  | 監視 widget 自動登録 item を user が Library 削除 → 現状は次 scan で復活。 `items.source_widget_id` / `source_entry_key` の back-link + `widget_item_hides` での除外記録 + reconcile を所有関係ベースに。 復元 UI は 500 | PH-CF-100, PH-CF-500 |
...
| 順 | PH         | 理由                                                                                                               | 依存                 |
| -- | ---------- | ------------------------------------------------------------------------------------------------------------------ | -------------------- |
| 1  | PH-CF-100  | データ整合 (孤立 item / ItemNotFound) + 監視アイテムの逆方向ライフサイクル data model。 後続 PH (300 / 500) の前提 | —                    |
...
| 5  | PH-CF-500  | 監視ウィジェット chrome + 除外アイテム復元 UI。 chrome は 400、 復元 UI は 100 の data model に依存                | PH-CF-100, PH-CF-400 |
...
| 監視アイテム逆方向ライフサイクル | `features/backend/item-service.md` / `features/backend/exe-scanner.md` / `features/widgets/_chrome-consistency.md` (or `projects.md` / `exe-folder.md`) | back-link 列の片肺検出 audit + Library 削除 → 再 scan で復活しない統合 test + 除外解除で復活する統合 test + 復元 UI の DOM 存在 e2e |
...
- **監視アイテムの逆方向ライフサイクル (確定、 2026-05-23 user 承認)**: フォルダ監視 / EXE フォルダ監視 が自動登録した Library アイテムを user が Library から削除したら、 当該 widget の除外リストに記録し **次の scan で復活させない** (現状の無言復活=モグラ叩きを解消)。 復元導線として widget 設定に「除外したアイテム一覧」 + 復元ボタン。 widget ごと削除すると除外リストも CASCADE で消える (fresh state)。 土台は既存 `widget_item_hides`、 穴 (items への back-link 不在 / Library 削除非連動 / exe-folder の key 空間ズレ) を直す。 スクリプト監視は Library 永続化しないため対象外。 data model = PH-CF-100、 復元 UI = PH-CF-500。
