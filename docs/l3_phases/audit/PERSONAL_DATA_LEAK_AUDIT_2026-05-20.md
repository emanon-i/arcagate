...
repo (`E:\Cella\Projects\arcagate`) に push 済の内容に対し、 user 個人情報 / 実 user DB 由来データ / 実 item 名 / secret が漏れていないかを machine grep + 画像目視で全件 audit。 修正は提案のみ、 PR 化はしない。
...
- ファイル:
  ...
  - `src-tauri/examples/seed_dev.rs:99` (`E:/Cella/Projects/arcagate`)
  - `src-tauri/examples/seed_dev.rs:111` (`C:/Users/<username>/Downloads`)
  - `src-tauri/examples/seed_dev.rs:125` (`E:/Cella/Projects`)
- 内容: `cargo run --example seed_dev` 用のダミーデータ投入スクリプトに、 user の Windows username と作業 drive (`E:/Cella/Projects`) を直書き
...
- ファイル:
  - `CLAUDE.md:138` (memory 永続 path `C:\Users\<username>\.claude\projects\E--Cella-Projects-arcagate\memory\`)
...
- ファイル:
  - `docs/l1_requirements/operations.md:308-309` / `:312` / `:331` / `:349` (`D:\secrets\arcagate\`)
  - `docs/l1_requirements/operations.md:323` (`E:\Cella\Projects\` も agent 触れる前提)
  - `docs/l3_phases/_archive/release-readiness/audit-final-r10.md:122` (`pnpm tauri signer generate -w D:\secrets\arcagate\arcagate.key`)
- 内容: signing 鍵保管場所として **個人 PC の D: 構成 (D:\secrets\arcagate\)** を doc に固定。 配布要件ではなく作業環境固有。
- リスク: 「私の signing key は D:\secrets\arcagate\ にある」 と公言しているのと等価。 鍵自体の盗難経路にはならないが、 attacker が social-engineering する際の手がかり (`gonda` username + D: drive + `secrets` folder 名 を組合せ可能)。
...
- ファイル:
  ...
  - `docs/l3_phases/_archive/PH-20260427-453_auto-kick-verify.md:18` (`C:/Users/<username>/.claude/projects/E--Cella-Projects-arcagate/<sessionId>.jsonl`)
  - `docs/l3_phases/_archive/PH-20260427-463_auto-kick-verify.md:19` (個別 sessionId UUID 付きの transcript path)
  - `docs/l3_phases/_archive/PH-20260425-287_codex-review-library.md:41` (worktree path 直書き `E:/Cella/Projects/arcagate/.claude/worktrees/interesting-engelbart-e6f255`)
  - `docs/l3_phases/_archive/PH-20260426-295_test-triage-ci-speed.md:89` (`E:/Cella/Projects/arcagate/.git/config`)
...
- 推奨対処:
  - 一括 sed で `C:/Users/gonda` → `$USERPROFILE`、 `E:/Cella/Projects/arcagate` → `<repo-root>` に generic 化
...
- **実 user item 名**: user の brand / 実 game 名 (oresam / 寝取らせ系 / すぱ日々 / 素晴らしき日々 / ぬきげみ / 味な島 / 貧乳 / ネトラレ系 / クロスドレッサー / リーマン駅 / りっち 等) を grep — **全件 0 hit**
- **個人 email**: `gonda452@gmail.com` を grep — **0 hit**。 検出された email は `test@test.com` (git config 設定 stub) と `x@y.com` (HTML fixture) のみ、 いずれも generic
...
```powershell
# 1. 個人 path
rg -n 'E:[\\/]Cella|C:[\\/]Users[\\/]gonda|\\\\Users\\\\\w+\\\\|/Users/\w+/' --hidden

# 2. user 実 item 名 (brand / 作品名)
rg -ni 'oresam|寝取らせ|すぱ日々|素晴らしき日々|ぬきげみ|味な島|貧乳|ネトラレ|クロスドレッサー|リーマン駅|りっち' --hidden
