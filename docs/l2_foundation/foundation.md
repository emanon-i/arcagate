...
Frontend vitest は T1-T4 plan で incremental に構築する ([`test_scenarios.md`](./test_scenarios.md) 参照)。
...
```
arcagate/
├── docs/                          # この doc 系 (L0-L3 layer model、docs/README.md 参照)
│   ├── l0_ideas/motivation.md     # L0 製品要求
│   ├── l1_requirements/           # vision.md + operations.md
│   ├── l2_foundation/
│   │   ├── foundation.md          # 本 file
│   │   ├── test_scenarios.md / lessons.md / design-tokens.md / i18n-policy.md / button-usage.md
│   │   ├── screens/               # 画面別 UX / IA カタログ
│   │   └── features/              # 機能契約 (backend / widgets / cross-cutting / screens)
│   └── l3_phases/
│       ├── _archive/              # 完了済 plan
│       ├── audit/ clean-feedback/ paid-quality/   # trace
│       └── _template/
│
...
| **Widget**           | Workspace に配置する mini app。 `src/lib/widgets/<name>/` で folder-per-widget 構成 (現行の種別集合はこの dir が単一情報源) |
