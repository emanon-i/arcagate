...
- これにより `CardOverrideDialog` overlay 下での `<img src>` 更新でも、 一覧 grid が遷移なしに
  切替わる。
- `LibraryCard` の `content-visibility: auto` 仮想化は撤廃済 (paint stale 起因の構造的欠陥が
  あったため)。 perf 影響は 690 cards の cold open で initial paint ~150ms (許容)、 scroll は
  60fps 維持。
- 旧 `LibraryView` の card 全体 `{#key item.icon_path|card_override_json}` 広域再 mount は撤廃済
  (paint stale 解消責務を ItemIcon に局所化したため、 card 全体再 mount は冗長)。
