...
PH-CF-1100 では `LibraryCard` の `content-visibility: auto` 仮想化を **撤廃** した:

- 旧実装: `freshIconMark` + `onMount` (PR #564) → `freshIconMark` + `$effect` (PR #570) の 2 段
  fix を入れたが、 (a) modal overlay 下で paint window が浪費される、 (b) `cmd_save_icon_file`
  の disk copy 遅延と 500ms / 1500ms 窓が competing、 など実 UI 経路で paint stale を残し続けた。
- 新実装: `content-visibility: auto` 自体を `.library-card` から除去。 grid 全 card を常時 paint
  する代わりに「即時反映」 を保証 (CLAUDE.md `<critical-rule id="instant-feedback">` 契約)。
  Perf 影響は 690 cards の cold open で initial paint ~150ms (許容)、 scroll は 60fps 維持。
...
- e2e: `tests/e2e/ph-cf-1100-appearance-state-mgmt.spec.ts`:
  - LB-2-real (② 真因経路): detail panel UI で toggle ON → 歯車 → 画像 picker を test hook で
    駆動 → 一覧カード `<img src>` が新 path に切替を verify
  - LB-5 (⑤): toggle OFF → カード `<img>` 要素が消滅 (fallback アイコンに切替) を verify
  - LB-6 (⑥): OFF 後の再 ON で `icon_path` + offsetX/Y が完全復元を verify
