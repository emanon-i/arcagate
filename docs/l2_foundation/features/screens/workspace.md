...
seed 解決は `resolveSeedCell` 1 関数に集約し、 (0,0) フォールバック禁止と viewport 中央
fallback を経路横断で保証する。
...
- `effectiveCols = Math.max(1, opts.cols ?? DEFAULT_GRID_COLS)`。 `widgetMaxRight` は **含めない**
  (含めると viewport 外に居座る widget が配置領域を広げ、 seed も新規 widget も viewport 外へ飛ぶ)。 viewport 外既存 widget は
  overlap 判定の障害物としては考慮するが、 新規 widget は 必ず viewport 内 (0..effectiveCols-w) に配置される。
