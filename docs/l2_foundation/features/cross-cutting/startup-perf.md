...
2026-05-27 までは旧 aspirational 値 cold 1500 / warm 1000 を hard gate として運用していたが、 CI runner 上では構造的に到達不能で perf workflow が常時赤かった。 「保守性 > 脆い perf チューニング」 方針に沿い、 実測 baseline (cold ~2450ms / warm ~2100ms 中央値) + ~33% buffer に再設定して **regression 検出器** として機能させる (`docs/l3_phases/paid-quality/audit/perf-budgets-2026-05-22.md` §2026-05-27 再設定 参照)。 数値そのものの改善 (WebView2 cold init / backend setup 短縮) は別 phase に分離。
