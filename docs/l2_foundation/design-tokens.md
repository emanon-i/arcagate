...
> **目的**: 「色 1〜2 色を決めるだけで色全体は色彩学で自動生成、 aesthetic (glass / neumorph / brutalist / HUD) は直交軸として token で表現」 という design system の構造定義。
...
LAYER 2  aesthetic primitives --radius/-surface/-shadow/-font/-bg-pattern/-decoration
   │     (色と直交、 glass/neumorph/brutalist/HUD を表現)
   ▼
...
| text 効果       | `--text-effect-glow` / `--text-effect-stroke` (HUD 等)                                  |
...
| **HUD**       | dark  | 黒地 + 黄/orange accent + monospace + radius 0 + text glow + bracket corner  |

Dark / Light は `.dark` class 切替 (flat、 `[data-theme]` を使わない)。 Neumorph / Brutalist / HUD は `[data-theme="theme-builtin-*"]` block。 旧 builtin (Cyan Steel / Coral Wine / Liquid Glass 等) は migration 032 で廃止。
