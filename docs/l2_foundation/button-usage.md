...
- **使用箇所** (= shadcn Button 不使用、 意図的 custom):
  - `TitleBar.svelte` window control (minimize / maximize / close)
  - `WidgetHandles.svelte` 8 方向 resize handle
  - `ItemFormCardOverride.svelte` color picker swatch (= 専門 UI)
  - `LibraryCard.svelte` card 本体 (= entire card が click target、 専門 UI)
- **理由**: OS 慣習尊重 / 専門 UI の visual unique 維持 / shadcn Button の token (`bg-primary` 等) と相性悪い
