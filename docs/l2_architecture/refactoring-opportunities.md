...
## Refactor Era 完走サマリ（batch-82 → 85）

| バッチ   | テーマ         | 主要成果                                                                                                                                              |
| -------- | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| batch-82 | 計測フェーズ   | exe 16.44 MB / フロント 556 KB / vitest 104 / cargo test 176 を baseline 化、混乱箇所 top 10 / refactor 候補リスト整備                                |
| batch-83 | 構造フェーズ   | 14 widget の folder-per-widget colocation、registry 化で WorkspaceLayout 33→9 行 / Sidebar 53→15 行                                                   |
| batch-84 | 簡素化フェーズ | WidgetSettingsDialog 583→95 行 + 7 dedicated Settings + 共通 1 件 / ConfirmDialog 共通化 3/3 / 防衛テスト 15 件追加 / vitest 119 / 119                |
| batch-85 | 性能フェーズ   | cargo bloat 上位 review + notify features 整理 / ThemeEditor lazy 化（メイン chunk -7 KB）/ 起動 P95 + idle memory 計測スクリプト整備（実機計測待ち） |

### 計測比較（batch-82 → 85）

| 指標                 | batch-82         | batch-85             | 変動                                                               |
| -------------------- | ---------------- | -------------------- | ------------------------------------------------------------------ |
| exe size             | 16.44 MB         | 16.5 MB              | ±0（vision 20 MB 余裕 +3.5 MB）                                    |
| メイン chunk         | 302.9 KB         | 362.4 KB             | +59 KB（widget colocation 後増加分）+ ThemeEditor 7.5 KB lazy 分離 |
| WidgetSettingsDialog | 583 行           | 95 行                | -488 行（83% 削減）                                                |
| confirm dialog 重複  | 3 箇所 ~270 行   | 単一 + 2 wrapper     | 大幅統合                                                           |
| vitest               | 104 件 / 11 file | 119 件 / 13 file     | +15（ConfirmDialog + registry）                                    |
| cargo test           | 176 件           | 176 件               | ±0                                                                 |
| 起動 P95             | 未計測           | 計測スクリプト整備済 | 実機計測待ち                                                       |
| idle memory          | 未計測           | 計測スクリプト整備済 | 実機計測待ち                                                       |

### Polish Era 起動条件チェック

`docs/l0_ideas/arcagate-engineering-principles.md` §9 の品質バー:

- ✅ exe / バンドル / 複雑度 baseline が揃った
- ⚠️ 起動 P95 / idle memory は計測スクリプト整備済、実機計測は次セッションで
- ✅ 規約機械化（PH-275/276/277/278/PH-373/PH-378）でリント逸脱は CI が止める
- ✅ 直近 4 バッチ（82〜85）で新規ユーザ指摘ナシ（自律運用継続中）

→ **Polish Era 起動可（実機計測待ちは Polish Era 内で消化）**。

### Polish Era 5 plan 提案（batch-86 候補）
