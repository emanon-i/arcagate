...
## 2026-04-22 Batch 16 E2E 修正 実行中

修正内容（PH-079〜083）:

- playwright.config.ts: globalTimeout 300s 追加
- workspace-editing.spec.ts: afterEach mouse.up() + dialog ボタン strict mode 修正
- widget-context-panel.spec.ts: item_id → itemId camelCase 修正
- widget-zoom.spec.ts: localStorage テスト初期化に reload 追加
- library-search.spec.ts: main 可視性チェック → searchInput 確認に変更
- library-empty-starred.spec.ts: starredIds 更新バグにより starred badge テストを skip

`pnpm test:e2e` 実行中 (19:xx)
