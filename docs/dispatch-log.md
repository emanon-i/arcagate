...
## セッション中断サマリ (2026-04-25) ← 次セッション最初に読め

### Git 状態

- **main**: batch-60 完了済み（PR マージ済み）
- **open PR**: #97（`claude/suspicious-varahamihira-ed628d`） ← **仕様違反・マージ禁止**
- **未 push**: なし（全 commit 済み）

### PR #97 の問題点（取り下げ or 全面修正必要）

| 問題          | 現状 PR #97             | 正しい仕様                            |
| ------------- | ----------------------- | ------------------------------------- |
| アスペクト比  | `aspect-square` (1:1)   | **4:3**（`aspect-[4/3]`）             |
| S/M/L の効果  | アイコンのみサイズ変化  | **カード全体サイズ**が変わる          |
| gap           | `gap-3`（12px）に誤変更 | **`gap-4`（16px）固定**・動かさない   |
| Settings 連動 | なし                    | Settings > Library セクションから操作 |

### batch-65 全残タスク（すべて未着手）

- 4:3 アスペクト比 + S/M/L でカード全体サイズ変更
- gap 固定 + 外側 padding 吸収グリッド（`repeat(auto-fill, Npx)` + `justify-center` で実現可、gapは固定のまま）
- 背景 3 モード（塗りつぶし / 画像 / なし）
- 画像: `object-cover` + focal point picker（`object-position` 制御）
- 文字色 color picker
- 縁取り（`text-stroke` または SVG テキスト）設定
- メタデータ表示有無（容量 / 起動回数 / 更新日時 等）
- Settings > Library セクション新設（現在 Settings > General のみ）

### 次セッション着手順序

1. PR #97 を **close**（仕様違反のため）
2. main から新ブランチ `claude/batch-65-library-card` を切る
3. Library カード全面再実装（4:3 / gap 固定 / Settings 連動 / 背景 3 モード / focal point 等）
4. `pnpm verify` → PR 作成
