...
- Endfield / Ubuntu 透過プリセット（batch-41/42 予定）
...
| 4  | ❌   | Workspace D&D 配置 / 移動      | ウィジェットの D&D 追加・移動が動かない（**最優先リグレッション**）      | PH-20260423-192 で修正予定                                                 |
| 5  | ❌   | ホットキー設定反映             | Settings でホットキーを変更して再起動しても反映されない                  | PH-20260423-193 で修正予定（cmd_set_hotkey に global_shortcut 再登録追加） |
| 6  | ❌   | クリック SE / サウンド         | パレット実行時に音が鳴らない                                             | PH-20260423-194 で修正予定（Web Audio API 接続確認）                       |
| 7  | ❌   | ウィジェットズームスライダー   | Settings > ズームスライダーを動かしても反映されない                      | PH-20260423-194 で修正予定（CSS var / reactive state 確認）                |
| 8  | ❌   | フォルダアイテム起動           | フォルダ系アイテムを実行しても開かない                                   | PH-20260423-194 で修正予定（explorer.exe パス渡し確認）                    |
| 9  | ⚠️    | ESC キーの挙動                 | 編集モード中 ESC を押すと確定になってしまう（キャンセルが期待挙動）      | PH-20260423-195 で修正予定                                                 |
| 10 | ⚠️    | ウィンドウリサイズ時レイアウト | ウィンドウを狭くするとウィジェットがオーバーラップする                   | PH-20260423-196 で修正予定（固定位置 + スクロール化）                      |
| 11 | ⚠️    | ボタンサイズ不安定             | テキスト変化（例: 確定↔キャンセル）に伴いボタンサイズがガタつく          | PH-20260423-197 で修正予定（min-w 固定）                                   |
| 12 | ⚠️    | モーダル背景が透明             | 削除確認 / リネームダイアログの背景オーバーレイがほぼ透明                | PH-20260423-197 で修正予定（backdrop opacity 調整）                        |
| 13 | 💄   | WorkspaceHintBar が小さい      | ヒントバーの文字・行が小さすぎて読みにくい                               | batch-41 以降で再デザイン予定                                              |
| 14 | 💄   | ★（スター）ボタン              | スターボタンが視覚的にトグル状態を示さない・押しても変化がわかりにくい   | batch-41 以降で改善予定                                                    |
| 15 | 💄   | アイコン / サムネイル表示      | アイテムアイコンが小さすぎる / サムネイルが見切れる                      | batch-41 以降で調整予定                                                    |

### 対応優先順位

1. **ブロッカー修正**（batch-41 hotfix, PH-192〜194）: D&D・ホットキー反映・SE/ズーム/フォルダ起動
2. **動作不具合**（同バッチ or PH-195〜197）: ESC 挙動・レイアウト崩れ・ボタンがたつき・モーダル背景
3. **UI 改善**（batch-42 以降）: HintBar 再デザイン・★ボタン・アイコンサイズ・パレット候補ナビ
