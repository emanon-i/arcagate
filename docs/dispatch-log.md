...
## 2026-04-28 [batch-107] done

### Goal達成: Widget UX 全面改修 完走

user dev fb (2026-04-27) 14 項目を 8 plan (PH-472〜PH-479) で fix、PR #175/176/181/182/184 merged。
追加で reactive 反映 audit + 7 E2E spec + helper 導入。

### PH-472〜PH-479 main 反映 commits

- 73b13c5 fix(batch-107 PH-479): reactivity audit + E2E + helper (#184)
- 59c0c3e feat(batch-107 PH-472): handle normalize (#176)
- f4666b2 feat(batch-107 PH-477): undo/redo (PH-473-477 cumulative squash) (#181)
- f269514 feat(batch-107 PH-478): edit state cleanup (#182)
- a783ddb docs(batch-107): 7 plan 起票 (#175)

### user fb 14 項目 → 7 シナリオ + audit 全部 fix

1. ハンドル独自実装やめて → PH-472 (shadcn 風 floating × button、選択時のみ)
2. グリッドでかい / 無限風 canvas / Crop → PH-473 (50% 縮小、+4 行余白、Crop button)
3. 重なるな (auto-rearrange 廃止) → PH-473 (配置拒否 + drop preview 赤)
4. ライブラリカード再利用 / 複数選択 / sort → PH-474 (LibraryItemPicker 全面リライト)
5. アイテム削除 cascade → PH-474 (Rust purge_item_from_widget_configs)
6. フォント統一 (sm 等 hardcode 撲滅) → PH-475 (text-ag-{xs..2xl} + audit script + lefthook + CI)
7. ウィンドウ半透明 → PH-476 MVP (window-vibrancy Mica/Acrylic)
8. Undo/Redo → PH-477 (Ctrl+Z, ring buffer 50)
9. キャンセル後の編集 state 残留 → PH-478 (race-free lock + history clear)
   10-15. **画面切替なしで反映** → PH-479 (App 全体 reactivity audit、items/workspace/theme spread copy + Promise.all reload + helper)

### 残課題 (batch-108 以降)

- per-workspace 背景画像 (PH-476 scope 外): batch-108 以降か単独 plan
- filter (PH-474 sort のみ): 同上
- Codex 5/Polish Era 続編 (panic_hook / KillSwitchDialog / 配信信頼性): batch-109 以降

### 重要 lessons (lessons.md 反映済)

- Svelte 5 store mutation で `array.map((x) => (x.id===id ? updated : x))` は keyed each で reactive 不発 → 全要素 spread copy
- 集約データ (counts/stats/widgets) は別 store にあるため mutation 後の明示 reload 必須
- Rust cascade 後の frontend store reload (dynamic import で循環依存回避)
- mutation+reload helper (`withReload(action, ...reloads)`) で呼び忘れ防止

### batch-107 → batch-108 移行
