...
Release / distribution / support の **operational requirement** を 1 file に統合。
...
ディスパッチ運用の **判断ルール** だけを集約（コマンド例 / フローチャート等は git log / gh pr / lefthook で自明なため載せない）。

---

## <severity>critical</severity> user-redo depth-first（最優先）

1 issue ずつ depth-first で回す。並行 plan / 並行 PR は禁止（詳細は Rule 1）。

### 1 issue の depth-first サイクル

1. **fact 確認**: コードの実態 / 再現手順 / screenshot / **root cause** まで特定（推測で plan 書かない）
2. **guideline 引用**: 該当する規範 doc (`docs/l2_foundation/design-tokens.md` / `i18n-policy.md` / `button-usage.md`、`vision.md` の UX 標準、該当 `screens/` `features/`) を引いて該当 section を Read (repo 外の memory `design_guidelines_index.md` が全体地図)
...
- 起点: `main`（direct branch off main、squash merge で戻す）
...
**Status**: active — auto-update 有効化。初回 `v0.1.0` リリースに向け鍵セットアップ。手順は §0 (クイック)。
**Scope**: arcagate (Tauri v2 + GH Releases auto-update) の updater 鍵運用 = **Tier 1 (auto-update 経路の完全性)**

**関連**: 配布元 attestation (**Tier 2**) は本 doc の [Part 2.3 Cosign Verification](#23-cosign-verification-tier-2-attestation) 参照。
Tier 2 は agent が CI に統合済 (R10-X PR)、user 作業 0 で運用中。本 Tier 1 と独立した信頼レイヤー。

---

## 0. セットアップ & 初回リリース クイック手順

> 本節は鍵生成 〜 初回 `v0.1.0` リリースまでの最短手順。**秘密鍵・passphrase は AI (Claude Code 等) に一切渡さない**前提 (理由は §4)。各 step の詳細は §1〜§12 を参照。
...
- **過去の落とし穴 / 再発防止**: [docs/l2_foundation/lessons.md](../l2_foundation/lessons.md)
...
開発に関わりたい場合は [README.md](../../README.md) の「開発」 節 + [AGENTS.md](../../AGENTS.md) を参照。
...
**→ 本 doc の [Part 2.2 Pubkey / Minisign Procedure](#22-pubkey--minisign-procedure)**
...
- `.claude/rules/workflow.md` の `label-content` 「同じ機能 = 同じ icon + 同じラベル」
