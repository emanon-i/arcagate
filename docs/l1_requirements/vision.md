...
> **凍結理由**: PH-004 で凍結。コンテキストメニュー統合（SendTo / Shell Extension）は優先度見直し待ち。

~~エクスプローラーの「送る」相当の統合（第1段階）。右クリックメニューに登録コマンドを動的サブメニューとして表示（第2段階）。複数ファイル選択時の受け渡し対応。~~

### ファイルマネージャー・AI連携深化 ⚠️ 凍結

- **ID**: REQ-20260226-013
- **優先度**: 凍結

> **凍結理由**: PH-004 で凍結。ファイルブラウザ・セマンティック検索は優先度見直し待ち。

~~タブ付きファイルブラウザ、インメモリインデックスによる高速ファジー検索（10万件規模1秒以内）、ファイルシステムインデックスのAI公開、セマンティック検索。~~
