...
### ~~MCP連携~~ → Agent-first CLI

- **ID**: REQ-20260226-011
- **優先度**: 対象外（除去済み）

PH-003-H で Agent-first CLI に移行。MCP サーバーは除去済み。Claude Code からは `arcagate_cli` を Agent tool / Skill 経由で直接呼び出す方式に変更。
...
> **v2 (2026-05-18〜)**: token システムは「seed 最小化 + 色彩学的自動派生 + aesthetic 直交軸」へ刷新。
...
2. **Aesthetic primitives**: 色と直交した素材感 (`--ag-radius-*` / `--surface-*` / `--shadow-*` / `--bg-pattern-*` / `--decoration-*` / `--font-family-*`)。 glass / neumorph / brutalist / HUD を表現。
...
BuiltinTheme（DB 保存、 seed + aesthetic は arcagate-theme.css 側で定義、 css_vars は空）
  ├── Dark      (id='dark':      dark ベース / glass aesthetic)
  ├── Light     (id='light':     light ベース / whisper aesthetic)
  ├── Neumorph  (id='neumorph':  light ベース / pastel solid + dual shadow)
