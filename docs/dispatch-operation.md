...
- PC 解錠後の自律実機確認も**ユーザの GO 待ち**。勝手に E2E や dev を起動しない
- 自動バッチ進行中（§2）はこの許可制が適用される
...
drag/drop / hover / keyboard を使うテストには `page.mouse.up()` を afterEach/afterAll で確実に呼ぶ。新規 E2E テストを追加する際は以下のパターンを守る:
...
workspace-editing.spec.ts など既存 drag テストにもこのガードを追加すること（§5 ブレーキ条件には含めないが、次バッチの整備対象）。

### タイムアウト強制終了

`pnpm test:e2e` は playwright.config.ts の `globalTimeout` で 5 分上限（300_000ms）を維持する。コマンドライン起動時も `timeout 300` 相当を意識すること。
