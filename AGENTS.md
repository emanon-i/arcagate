...
1. **[docs/l0_ideas/motivation.md](docs/l0_ideas/motivation.md)** — L0 製品要求 (なぜ / 何 / 誰 / Non-goals / 成功条件 / 制約 / 利用形態 / 失敗パターン)
...
5. **[docs/l2_foundation/lessons.md](docs/l2_foundation/lessons.md)** — 過去の失敗 / 再発防止 (メタ教訓のみ、 個別 bug は git log)
...
- `pnpm app:dev` — **隔離 dev 起動 (推奨)**。`tauri.dev.conf.json` で identifier を `com.arcagate.desktop.dev` に上書き → データ保存先 (`%APPDATA%\com.arcagate.desktop.dev\`) が本番 (`...desktop\`) と物理分離。インストール版を daily-use しながら dev しても**本番データに一切触らない**。素の `pnpm tauri dev` は本番 identity (= 本番データ) を使うので避ける
- `pnpm tauri dev` — 非隔離の開発起動。 CDP 経由で agent も操作する場合 `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port=9515` を付ける (user dev port と被らないよう agent は別 port、 別 worktree、 別 `WEBVIEW2_USER_DATA_FOLDER` で隔離)
...
本 repo は **独立 module**。 外部 monorepo / parent repo に依存しない (本 repo root 配下のみで完結)。

memory (永続) は `$USERPROFILE\.claude\projects\<repo-slug>\memory\` (repo 外、 agent session 跨ぎで参照)。 詳細は `memory/MEMORY.md` 参照。
...
- skill 詳細: `$USERPROFILE\.claude\skills\run-codex\SKILL.md`
- 軽量 fix の事実確認 / plan 文書化にも活用して Opus quota 節約 (上記 `ops` rule)

---

## LLM agent navigation index

旧 `docs/llms.txt` を本 AGENTS.md に統合 (2026-05-13)。 LLM agent が repo を navigate する時の全 entry point:

### L0 (要求 / 動機、 stable layer)

- [`docs/l0_ideas/motivation.md`](docs/l0_ideas/motivation.md) — (a) なぜ作る / (b) 何 / (c) 誰のため / (d) Non-goals / (e) 成功条件 / (f) 制約 / (g) 利用形態 / (h) 失敗パターン
...
- [`RELEASE.md`](RELEASE.md) — **リリース手順 (Claude Code 単独実行可)**。version bump → tag → workflow → 検証 → publish の runbook。tier (lean/full) / 署名 / 自動アップデートの実務はここ

### L2 (基本設計)

- [`docs/l2_foundation/foundation.md`](docs/l2_foundation/foundation.md) — 全体アーキテクチャ overview / 技術 stack / レイヤー設計 / IPC 境界 / Frontend state / Error / SQLite schema / dirs / 設計判断 / 非機能 / CI/CD / 用語
- [`docs/l2_foundation/test_scenarios.md`](docs/l2_foundation/test_scenarios.md) — テストシナリオ ⇄ 実装 link、 T1-T4 plan + scenario 一覧
- [`docs/l2_foundation/lessons.md`](docs/l2_foundation/lessons.md) — 失敗駆動メモリ・再発防止パターン
- [`docs/l2_foundation/screens/`](docs/l2_foundation/screens/):
  - [`palette.md`](docs/l2_foundation/screens/palette.md) / [`library.md`](docs/l2_foundation/screens/library.md) / [`workspace.md`](docs/l2_foundation/screens/workspace.md) / [`settings.md`](docs/l2_foundation/screens/settings.md) / [`onboarding.md`](docs/l2_foundation/screens/onboarding.md)

### L3 (実装計画、 アーカイブ済)

- [`docs/l3_phases/_archive/`](docs/l3_phases/_archive/) — 完了済 plan (556 file)
